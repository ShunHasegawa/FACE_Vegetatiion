...
coeftbl <- data.frame(cbind(confint(c4d_m0_avg, full = TRUE), estimate = coef(c4d_m0_avg, full = TRUE))) %>% 
  transmute(predictor = row.names(.),
            estimate  = round(estimate, 3),
            CI        = paste0("[", round(.[, 1], 3), ", ", round(.[, 2], 3), "]"),
            imp       = c("-", round(importance(c4d_m0_avg), 3)))
pander(coeftbl)
```
...
coeftbl <- data.frame(cbind(confint(c3d_m0favg, full = TRUE), estimate = coef(c3d_m0favg, full = TRUE))) %>% 
  transmute(predictor = row.names(.),
            estimate  = round(estimate, 3),
            CI        = paste0("[", round(.[, 1], 3), ", ", round(.[, 2], 3), "]"),
            imp       = c("-", round(importance(c3d_m0favg), 3)))
pander(coeftbl)
```
