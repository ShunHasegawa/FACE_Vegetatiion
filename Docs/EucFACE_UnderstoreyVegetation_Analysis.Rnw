...
There are a number of potentail environmental variables to be used, for example soil chemistry and properties such as organic matter, moisture, temperature etc. Some variables are not complete for three years. Some variables are highly correlated one another and really redundant, so the following variables will be used.
...
\noindent check multicolliarity of the fitted variables
...
\section{Summary}
\noindent Repeate the above procedures for each year. As a result, important environmental drivers are identified as follows.
...
I also anlysed the following response variables using linear-mixed effects models (LMM).
\begin{itemize}
  \item Species richness
  \item Diversity (Shannon-Weiver)
  \item Species evenness
  \item Magunitude of yearly shift (dissimilarity between years)
  \item Proportion of plant functional groups (e.g. C$_3$ vs. C$_4$ plants)
\end{itemize}

\subsection{C3 vs C4 plant}
\noindent This is propotation data, but logit transformation seems to work quite well.
...
\subsubsection{LMM}
\noindent GLMM indicated a CO2 x Year interaction, but it doeesn't provide reliable p-values. So I also run LMM, folling logit transformation.
