...
plot(s_c4_ddiff ~ c4_ddiff, data = c34sum)

c4d_m0_slp_9  <- lmer(s_c4_ddiff ~ co2*(s_logmiost + s_temp)+(0+s_logmiost|ring/id), data = c34sum, REML = F)
c4d_m0_slp_10 <- lmer(s_c4_ddiff ~ co2*(s_logmiost + s_temp)+(1|ring/id), data = c34sum, REML = F)
summary(c4d_m0_slp_9)
summary(c4d_m0_slp_10)
r.squaredGLMM(c4d_m0_slp_9)
r.squaredGLMM(c4d_m0_slp_10)

anova(c4d_m0_slp_9, c4d_m0_slp_10)
AICc(c4d_m0_slp_9, c4d_m0_slp_10)
c4d_m0_slp_11 <- lmer(s_c4_ddiff ~ 1+(0+s_logmiost|ring/id), data = c34sum)
summary(c4d_m0_slp_11)
r.squaredGLMM(c4d_m0_slp_11)
r.squaredGLMM(c4d_m0_slp_9)


c4d_m0_slp_11 <- aov(s_c4_ddiff ~ co2*(s_logmiost + s_temp), data = c34sum)
...
ggplot(c34sum, aes(x = s_logmiost, y = s_c4_ddiff, group = id))+
  geom_point()+
  geom_smooth(method = "lm", se = F)+
  facet_wrap(~ ring)

ggplot(c34sum, aes(x = s_logmiost, y = s_c4_ddiff, col = ring))+
  geom_point()+
  geom_smooth(method = "lm", se = F)
