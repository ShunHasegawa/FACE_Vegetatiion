...
# PRC ---------------------------------------------------------------------
grass_DS_prc <- grass_DS_sum %>% 
  select(-PFG, -type) %>% 
  spread(pfg_type, value) %>% 
  arrange(id, year)
prc_ds_spp <- grass_DS_prc[, c("D_c3", "D_c4", "S_c3", "S_c4")]
...
grass_Sc4 <- veg_FullVdf %>% 
  filter(form == "Grass") %>% 
  group_by(year, ring, co2, RY, form, PFG, variable) %>% 
  summarise(value = sum(value)) %>% 
  left_join(grass_df[, c("species", "type")], by = c("variable" = "species")) %>% 
  filter(type == "S" & PFG == "c4") %>% 
...
names(grass_Sc4)
sc4 <- c("Axonopus.fissifolius", "Cymbopogon.refractus", "Eragrostis.brownii", 
         "Eragrostis.leptostachya", "Fimbristylis.dichotoma", "Paspalum.dilatatum", "Setaria.parviflora")


sc4_dd <- veg_FullVdf %>% 
  filter(variable %in% sc4) %>% 
  group_by(variable, year, ring, co2, plot, id, RY) %>% 
  summarise(value = sum(value)) %>% 
  ungroup()
sc4_d0 <- sc4_dd %>% 
  filter(year == "Year0") %>% 
  rename(value0 = value) %>% 
  select(variable, id, value0)
sc4_dd_ed <- filter(sc4_dd, year != "Year0") %>%
  left_join(sc4_d0)

sc4_ms <- dlply(sc4_dd_ed, .(variable), function(x){
  lmer(log(value + 1) ~ co2 * year + log(value0 + 1) + (1|ring) + (1|id) + (1|RY), data = x)
})
llply(sc4_ms, function(x) Anova(x, test.statistic = "F"))
ggplot(sc4_dd_ed, aes(x = log(value0 + 1), y = log(value + 1), col = co2))+
  geom_point() +
  facet_wrap( ~ variable, scale = "free")

ggplot(sc4_dd_ed, aes(x = sqrt(value0), y = sqrt(value), col = co2))+
  geom_point() +
  facet_wrap( ~ variable, scale = "free")


prc_ds_spp <- log(grass_Sc4[, sc4] + 1)
prc_ds_sit <- grass_Sc4[, c("year", "ring", "co2")]

# prc_ds <- capscale(prc_ds_spp ~  year * co2 + Condition(year), data = prc_ds_sit, distance = "euclidean")
prc_ds <- prc(prc_ds_spp, prc_ds_sit$co2, prc_ds_sit$year)
# prc_ds_res <- anova(prc_ds, permutations = alperms_bind[sample(allperm_n, 4999), ], by = "axis", parallel = 3)

ctrl <- how(plots = Plots(strata = prc_ds_sit$ring, type = "free"),
            within = Within(type = "series"), nperm = 999)
prc_ds_res <- anova(prc_ds, permutations = ctrl, first = TRUE)
prc_ds_res
plot(prc_ds)
?prc

rowSums(prc_ds_spp)
pa <- adonis(prc_ds_spp ~ co2 + year, data = prc_ds_sit[-5, ],nperm = ctrl)
pa
