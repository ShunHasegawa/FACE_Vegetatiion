...
prc_sp1   <- decostand(PlotSumVeg[, SppName], method = "log") # log-transformed sp df
prc_sp2   <- prc_sp1 %>% 
...
# prc_site <- RingSumVeg %>%                                   # site df 
#   select(year, ring, co2) 

prc_site <- prc_sp2 %>%                                   # site df 
  select(year, ring, co2) 
...
cntrol_year <- how(within = Within(type = "series"),                     # year within plot is a time seiries
                   plot   = Plots(strata = prc_site$ring, type = "free"),  # plot should not be exchanged
                   nperm = 4999)  
...
llply(split(perm_year[sample(nrow(perm_year), 1), ], prc_site$ring), function(x){
  m <- matrix(x, ncol = 4)
  apply(m, c(1, 2),  function(x) paste(prc_site$ring, prc_site$year, sep = "-")[x])
})


# run permutation test
prc_res <- anova(prc_all, permutations = perm_year[sample(4999), ], by = "axis")
prc_res <- anova(prc_all, permutations = perm_year[sample(999), ], by = "axis")
prc_res
...
mds_arrw_d <- data.frame(scores(mds_envfit, "vectors"), 
  ...
  filter(pval <= 0.1)
boxplot(CAP1 ~ co2 * year, data = res_prc_site_ring)
...
year0_df        <- RingSumVeg %>%                                               # df for Year0
  filter(year == "Year0") %>% 
...
res_pric_sp_d2 <- res_pric_sp_d %>%
  ...
  mutate(measure = factor(measure, levels = c("pfgform", "origin"), labels = c("PFG", "Origin")),
         ...
         type    = recode(type, c4Grass = "C4 grass", c3Grass = "C3 grass",
                          Forb = "Non_legume", legume = "Legume", native = "Native", 
