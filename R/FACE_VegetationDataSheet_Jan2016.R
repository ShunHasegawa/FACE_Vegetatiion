# Here, I'm going to create a datasheet for vegetation survey at FACE in 2016
# based on the last year result (Year2)
...
#################
# plot sum
plot.sum <- ddply(vegYear3, .(ring, plot), function(x) colSums(x[,PlntCol]))
...
#############################
# ring sum
ringsum <- ddply(vegYear3, .(ring), function(x) colSums(x[,PlntCol]))
...
ring.veg <- split(vegYear3,list(vegYear3$ring, vegYear3$plot))
...
for (i in ival){
  for (j in c("A","B","C","D")){
    ...
    labs <- vector()
    labs[c(1,16,20,25,31,41)] <- c("EucFace understory vegetation survey Jan 2016",
                                   "Date:",
