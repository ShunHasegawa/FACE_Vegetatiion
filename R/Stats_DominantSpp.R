...
#Paspalidium.distans----
DmSpp[6]
pdDF <- subsetD(SppPlotSum, variable == "Paspalidium.distans")
bxplts(value = "value", xval = "co2", ofst = 1, data = pdDF)

m1 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson, data = pdDF)
m2 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = sqrt), data = pdDF)
m3 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = power(1/3)), data = pdDF)
l_ply(list(m1, m2), overdisp.glmer)
# slightly overdespersed. 
m4 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id) + (1|obs), 
            family = poisson, data = pdDF)
m5 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id) + (1|obs), 
            family = poisson(link = sqrt), data = pdDF)
m6 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id) + (1|obs), 
            family = poisson(link = power(1/3)), data = pdDF)
l_ply(list(m4, m5), overdisp.glmer)
# use m5
Anova(m5)
plot(m5)
qqnorm(resid(m5))
...
# compare AIC
CompAIC_pd <- CompAIC(m5)
CompAIC_pd

# co2xtime interaction
m7 <- lmer(sqrt(value + 1) ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
           data = pdDF)
AnvF_pd <- Anova(m7, test.statistic = "F")
AnvF_pd
...
m6 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id) + (1|obs), 
            family = poisson(link = power(1/3)), data = gsDF)
l_ply(list(m4, m5, m6), overdisp.glmer)
# use m5 as m4 and m5 had convergense problem
Anova(m5)
plot(m5)
qqnorm(resid(m5))
qqline(resid(m5))
...
# LMM
m5lmr <- lmer(sqrt(value + 1) ~ year * co2 + (1|block) + (1|ring)  + (1|id), data = gsDF)
plot(m5lmr)
qqnorm(resid(m5lmr))
qqline(resid(m5lmr))
AnvF_gs <- Anova(m5lmr, test.statistic = "F")
AnvF_gs

#Sida.rhombifolia----
DmSpp[8]
srDF <- subsetD(SppPlotSum, variable == "Sida.rhombifolia")
bxplts(value = "value", xval = "co2", ofst = 1, data = srDF)

m1 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson, data = srDF)
m2 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = sqrt), data = srDF)
m3 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = power(1/3)), data = srDF)
l_ply(list(m1, m2, m3), overdisp.glmer)
...
# use m6
Anova(m6)
### something is weird... not sure why.. use m5 instead.
Anova(m5)
plot(m5)
qqnorm(resid(m5))
qqline(resid(m5))
# compare AIC
CompAIC_sr <- CompAIC(m5)
CompAIC_sr

# LMM
m5lmr <- lmer(sqrt(value + 1) ~ year * co2 + (1|block) + (1|ring)  + (1|id), data = srDF)
plot(m5lmr)
qqnorm(resid(m5lmr))
qqline(resid(m5lmr))
AnvF_sr <- Anova(m5lmr, test.statistic = "F")
AnvF_sr

#Centella.asiatica----
DmSpp[9]
caDF <- subsetD(SppPlotSum, variable == "Centella.asiatica")
bxplts(value = "value", xval = "co2", ofst = 1, data = caDF)

m1 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson, data = caDF)
m2 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = sqrt), data = caDF)
m3 <- glmer(value ~ year * co2 + (1|block) + (1|ring)  + (1|id), 
            family = poisson(link = power(1/3)), data = caDF)
l_ply(list(m1, m2, m3), overdisp.glmer)
# m3
Anova(m3)
# power(1/3) doesn't work..?

# try different optimizer
ms <- llply(list(m1, m2, m3), function(x) update(x, control = glmerControl(optimizer = "bobyqa")))
l_ply(list(m1, m2, m3), overdisp.glmer)
...
# compare AIC
CompAIC_ca <- CompAIC(m4)
CompAIC_ca

# indication of co2xyear
m1lmr <- lmer(log(value + 1) ~ year * co2 + (1|block) + (1|ring)  + (1|id), data = caDF)
plot(m1lmr)
qqnorm(resid(m1lmr))
qqline(resid(m1lmr))
AnvF_ca <- Anova(m1lmr, test.statistic = "F")
AnvF_ca
